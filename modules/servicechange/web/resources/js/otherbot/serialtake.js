...
//        setTimeout(function(){jQuery('.like_tt').css("display","none");thisEl._searthAnek();},3000);
    ...
    _searthFilm:function(){
        var thisEl=this;
        jQuery('.mainprasp tr.serin').each(function(ind,el){var el1=jQuery(el);var t={'name':el1.find("div.serinf a").text()+" "+el1.find("div.serinf .rasorname").text()+"",
            'img':(thisEl.url+el1.find('.lsmtb a img').attr('src')).replace("ru//images/mini","ru/images"),'film':$('.mainprasp h3').text().replace(", выходят серии следующих сериалов:","."),
            'discription':el1.find("div.serinf a").text()+" "+el1.find(".rsdtv").text()};thisEl.saveFilm(t);});
